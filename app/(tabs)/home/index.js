...
const index = () => {
  ...
  useEffect(() => {
    const fetchAllPosts = async () => {
      try {
        const response = await axios.get("http://192.168.148.29:4444/all");
  ...
  const handleLikePost = async (postId) => {
    try {
      const response = await axios.post(
        `http://192.168.148.29:4444/like/${postId}/${userId}`
      );
    ...
    <ScrollView>
      ...
      <View>
        {posts?.map((item, index) => (
          <View key={index} style={{borderBottomColor:'black',borderBottomWidth:0.8}}>
            <View
              style={{
                flexDirection: "row",
